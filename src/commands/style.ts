...
export async function styleCommand(args: string[]): Promise<void> {
  ...
  fs.writeFileSync(absolutePath, DEFAULT_GLOBAL_CSS);

  section("Styles Configured", successText(`Struct UI default styles applied to ${finalPath}`));
  divider();
  console.log(indent(mutedText("Try changing the theme with 'npx sui palette <theme>'")));
}
